...
| **Image size** | S, M, L, or Fill. Circle defaults to M; other shapes default to Fill. Below large screens S/M/L use a fraction of the column; from `lg` they fill the column up to a max width. |
| **Horizontal alignment** | Left, center, or right. Applies when the image is narrower than the cell. |
| **Vertical alignment** (cell) | Row (inherit), top, center, or bottom. Overrides the row setting for that image. |
...
| **Vertical alignment** (row) | Top, center, or bottom. Default for every cell; an image can override it. |
